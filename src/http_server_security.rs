...
impl Claims {
    ...
    pub fn new(user_id: String, roles: Vec<String>, permissions: Vec<String>, exp_hours: i64) -> Self {
...
impl AuthValidator {
    ...
    pub fn validate_permission(&self, token: &str, required_permission: &str) -> Result<bool, String> {
        let claims = self.validate_api_key(token)?;
...
mod tests {
    ...
    fn test_jwt_extract_bearer_token() {
        ...
        headers.insert(
            "Authorization",
            "Bearer my_token_123".parse().unwrap()
        );
    ...
    fn test_jwt_config_custom() {
        let config = JwtConfig::new("my_secret_key".to_string())
            .with_expiration(48);
