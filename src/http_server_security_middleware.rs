...
use crate::http_server_security::{
    RateLimiter, RequestSizeLimiter, AuthValidator, SecurityLogger, InputValidator,
};
...
pub async fn request_size_middleware(
    ...
    match size_limiter.validate_request(headers, body_size, url_length) {
        ...
        Err(status) => {
            let ip = extract_ip(&request);
            SecurityLogger::log_event("REQUEST_SIZE_LIMIT", &format!("Body: {} bytes, URL: {} chars", body_size, url_length), Some(&ip.to_string()));
            Response::builder()
                .status(status)
        ...
        for param in query.split('&') {
            if let Some((key, value)) = param.split_once('=') {
                ...
                if let Err(e) = InputValidator::validate_string(key, 100) {
                    SecurityLogger::log_invalid_input(&ip.to_string(), &format!("Query key: {}", e));
                ...
                if let Err(e) = InputValidator::validate_string(value, 1000) {
                    SecurityLogger::log_invalid_input(&ip.to_string(), &format!("Query value: {}", e));
                    return Response::builder()
                        .status(StatusCode::BAD_REQUEST)
                        .body(axum::body::Body::from("Invalid query parameter"))
                        .unwrap();
    ...
    for (name, value) in headers.iter() {
        if let Ok(value_str) = value.to_str() {
            if let Err(e) = InputValidator::validate_string(value_str, 1000) {
                SecurityLogger::log_invalid_input(&ip.to_string(), &format!("Header {}: {}", name, e));
                return Response::builder()
                    .status(StatusCode::BAD_REQUEST)
