...
impl SecureSession {
    ...
    pub fn refresh(&mut self) {
        ...
        if self.expires_at - now < (2 * 60 * 60) {
    ...
    fn get_permissions_for_roles(roles: &[String]) -> Vec<String> {
        ...
        for role in roles {
            match role.as_str() {
                ...
                "user" => {
                    permissions.extend_from_slice(&["read".to_string(), "write".to_string()]);
                }
                _ => {
...
impl SecureUserStore {
    ...
    pub fn authenticate(
        ...
        // Create session
        let session =
            SecureSession::new(user.id.clone(), user.roles.clone(), ip_address, user_agent);
