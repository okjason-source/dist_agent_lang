...
#[cfg(feature = "web3")]
pub fn use_mold(mold_id: u64, base: &Path, name_override: Option<&str>) -> Result<String, String> {
...
fn mold_info_to_value(info: &crate::mold::MoldInfo) -> Value {
    ...
    map.insert(
        "creator".to_string(),
        Value::String(format!("{:?}", info.creator)),
    );
    map.insert(
        "ipfs_hash".to_string(),
        Value::String(info.ipfs_hash.clone()),
    );
    ...
    map.insert(
        "max_use_count".to_string(),
        Value::Int(info.max_use_count as i64),
    );
...
fn mold_config_to_value(mold: &MoldConfig) -> Value {
    ...
    agent_map.insert(
        "type".to_string(),
        Value::String(mold.agent.agent_type.clone()),
    );
    ...
    agent_map.insert("learning".to_string(), Value::Bool(mold.agent.learning));
    agent_map.insert(
