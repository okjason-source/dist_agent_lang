...
impl ConfigManager {
    ...
    pub fn get_env(key: &str, validation_rule: Option<ConfigValidationRule>) -> Result<Value, String> {
    ...
    pub fn store_secret(&mut self, key: &str, value: &str, encryption_key: &str) -> Result<(), String> {
        ...
        self.encrypted_secrets.insert(key.to_string(), secret.encrypted_value);
        Ok(())
    }

    /// Retrieve decrypted secret
    pub fn get_secret(&mut self, key: &str, encryption_key: &str) -> Result<String, String> {
        let encrypted_value = self.encrypted_secrets.get(key)
    ...
    pub fn get_database_config() -> Result<HashMap<String, Value>, String> {
        ...
        config.insert("port".to_string(), Self::get_env_or_default("DB_PORT", Value::Int(5432)));
        ...
        config.insert("password".to_string(), Self::get_required_env("DB_PASSWORD")?);
        config.insert("ssl_mode".to_string(), Self::get_env_or_default("DB_SSL_MODE", Value::String("require".to_string())));
    ...
    pub fn get_api_config() -> Result<HashMap<String, Value>, String> {
        ...
        config.insert("base_url".to_string(), Self::get_required_env("API_BASE_URL")?);
        config.insert("timeout".to_string(), Self::get_env_or_default("API_TIMEOUT", Value::Int(30000)));
        config.insert("retry_count".to_string(), Self::get_env_or_default("API_RETRY_COUNT", Value::Int(3)));
    ...
    pub fn get_blockchain_config() -> Result<HashMap<String, Value>, String> {
        ...
        config.insert("rpc_url".to_string(), Self::get_required_env("BLOCKCHAIN_RPC_URL")?);
        config.insert("chain_id".to_string(), Self::get_required_env("BLOCKCHAIN_CHAIN_ID")?);
        config.insert("private_key".to_string(), Self::get_required_env("BLOCKCHAIN_PRIVATE_KEY")?);
        config.insert("gas_limit".to_string(), Self::get_env_or_default("BLOCKCHAIN_GAS_LIMIT", Value::Int(21000)));
        config.insert("gas_price".to_string(), Self::get_env_or_default("BLOCKCHAIN_GAS_PRICE", Value::Float(20.0)));
    ...
    pub fn get_ai_config() -> Result<HashMap<String, Value>, String> {
        ...
        config.insert("model".to_string(), Self::get_env_or_default("AI_MODEL", Value::String("gpt-4".to_string())));
        config.insert("temperature".to_string(), Self::get_env_or_default("AI_TEMPERATURE", Value::Float(0.7)));
        config.insert("max_tokens".to_string(), Self::get_env_or_default("AI_MAX_TOKENS", Value::Int(1000)));
    ...
    fn validate_value(value: &str, rule: &ConfigValidationRule) -> Result<(), String> {
        ...
        if let Some(allowed) = &rule.allowed_values {
            if !allowed.contains(&value.to_string()) {
                return Err(format!("Value '{}' not in allowed values: {:?}", value, allowed));
            }
        }
        ...
        match rule.validation_type {
            ValidationType::Integer => {
                value.parse::<i64>().map_err(|_| "Value is not a valid integer".to_string())?;
            }
            ValidationType::Float => {
                value.parse::<f64>().map_err(|_| "Value is not a valid float".to_string())?;
            }
