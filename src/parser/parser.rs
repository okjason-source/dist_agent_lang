...
use crate::parser::ast::{
    ...
    FieldVisibility, ForInStatement, BreakStatement, ContinueStatement, LoopStatement,
...
impl Parser {
    ...
    fn parse_range(&mut self, position: usize, depth: usize) -> Result<(usize, Expression), ParserError> {
        let mut current_position = position;
    ...
    fn parse_object_literal(&mut self, position: usize, depth: usize) -> Result<(usize, HashMap<String, Expression>), ParserError> {
        if depth > Self::MAX_RECURSION_DEPTH {
            let (line, _column) = self.get_token_position(position);
            return Err(ParserError::SemanticError {
                message: format!(
        ...
        loop {
            ...
            match token {
                ...
                Some(Token::Identifier(key)) => {
                    ...
                    let (new_position, value) = self.parse_expression_with_depth(current_position, depth + 1)?;
                    current_position = new_position;
    ...
    fn parse_array_literal(&mut self, position: usize, depth: usize) -> Result<(usize, Vec<Expression>), ParserError> {
        if depth > Self::MAX_RECURSION_DEPTH {
            let (line, _column) = self.get_token_position(position);
            return Err(ParserError::SemanticError {
                message: format!(
        ...
        while let Some(token) = self.tokens.get(current_position) {
            match token {
                ...
                _ => {
                    ...
                    let (new_position, expr) = self.parse_expression_with_depth(current_position, depth + 1)?;
                    current_position = new_position;
                    elements.push(expr);
    ...
    fn parse_break_statement(&mut self, position: usize) -> Result<(usize, Statement), ParserError> {
    ...
    /// Parse `continue;`
    fn parse_continue_statement(&mut self, position: usize) -> Result<(usize, Statement), ParserError> {
    ...
    fn parse_match_statement(&mut self, position: usize) -> Result<(usize, Statement), ParserError> {
        ...
        while current_position < self.tokens.len() {
            ...
            if let Some(Token::Keyword(Keyword::Default)) = self.tokens.get(current_position) {
                let (new_position, _) = self.expect_token(current_position, &Token::Keyword(Keyword::Default))?;
                current_position = new_position;
                
                // Expect fat arrow
                let (new_position, _) = self.expect_token(current_position, &Token::Punctuation(Punctuation::FatArrow))?;
                current_position = new_position;
                
                // Parse default body - can be a block, a statement (break/continue), or a simple expression
                let body = if let Some(Token::Punctuation(Punctuation::LeftBrace)) = self.tokens.get(current_position) {
                    // It's a block
                    ...
                    let value = if matches!(self.tokens.get(pos), Some(Token::Punctuation(Punctuation::Comma)) | Some(Token::Punctuation(Punctuation::RightBrace))) {
                        None
                    } else {
                    ...
                    block
                } else if let Some(Token::Keyword(Keyword::Continue)) = self.tokens.get(current_position) {
        ...
        Ok((current_position, Statement::Match(MatchStatement {
            expression,
            cases,
            default_case,
        })))
    ...
    fn parse_match_pattern(&mut self, position: usize) -> Result<(usize, MatchPattern), ParserError> {
        ...
        if let Some(Token::Literal(ref lit)) = self.tokens.get(current_position) {
            if let Some(Token::Punctuation(Punctuation::DotDot)) = self.tokens.get(current_position + 1) {
                ...
                // Expect DotDot
                let (new_pos2, _) = self.expect_token(new_pos, &Token::Punctuation(Punctuation::DotDot))?;
                new_pos = new_pos2;
                
                // Parse end literal
                if let Some(Token::Literal(ref end_lit)) = self.tokens.get(new_pos) {
                    let end = Expression::Literal(end_lit.clone());
                    return Ok((new_pos + 1, MatchPattern::Range(Box::new(start), Box::new(end))));
                } else {
            ...
            match self.tokens.get(current_position) {
                Some(Token::Punctuation(Punctuation::At)) => {
                    ...
                    while attr_position < self.tokens.len() {
                        if let Some(Token::Punctuation(Punctuation::At)) = self.tokens.get(attr_position) {
                    ...
                    if attr_position < self.tokens.len() {
                        match self.tokens.get(attr_position) {
                            Some(Token::Keyword(Keyword::Fn)) => {
                                ...
                                let (new_position, mut method) = self.parse_function_statement(attr_position)?;
                        ...
                        return Err(ParserError::SemanticError {
                            message: "Attributes must be followed by 'fn' (function) or field name".to_string(),
                            line,
                            context: ErrorContext::new(),
                        });
                    }
    ...
    fn validate_service_security(&self, service: &ServiceStatement) -> Result<(), ParserError> {
        ...
        for method in &service.methods {
            let func_attr_names: Vec<&str> = method.attributes.iter()
                .map(|a| a.name.as_str())
                .collect();
            ...
            if func_has_secure && func_has_public {
                ...
                let line = if let Some(pos) = self.find_function_position(&service.name, &method.name) {
