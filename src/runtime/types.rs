...
impl Type {
    ...
    pub fn substitute_generic(&self, generic_name: &str, concrete_type: &Type) -> Type {
        match self {
            ...
            Type::Option(inner_type) => Type::Option(Box::new(
                inner_type.substitute_generic(generic_name, concrete_type),
            )),
            Type::List(inner_type) => Type::List(Box::new(
                inner_type.substitute_generic(generic_name, concrete_type),
            )),
            ...
            Type::Set(inner_type) => Type::Set(Box::new(
                inner_type.substitute_generic(generic_name, concrete_type),
            )),
            _ => self.clone(),
        }
    ...
    pub fn is_compatible_with(&self, other: &Type) -> bool {
        match (self, other) {
            ...
            (Type::List(inner1), Type::List(inner2)) => inner1.is_compatible_with(inner2),
...
impl TypeEnvironment {
    ...
    pub fn get_struct_fields(&self, name: &str) -> Option<&HashMap<String, Type>> {
        self.structs.get(name).or_else(|| {
            self.parent
                .as_ref()
                .and_then(|parent| parent.get_struct_fields(name))
        })
    }
}
