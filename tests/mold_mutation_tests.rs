...
fn test_memory_limit_bytes_exact() {
    ...
    assert_eq!(
        result, expected,
        "8192 bytes should be {} pages, got {}",
        expected, result
    );
    assert_eq!(result, 2);
}
...
fn test_memory_limit_division_by_page_size() {
    ...
    assert!(
        result < 10_000_000,
        "Result should be reasonable page count"
    );
}
...
fn test_memory_limit_multiplication_chain() {
    ...
    assert_eq!(
        result_2gb,
        result_1gb * 2,
        "2GB should be exactly 2x 1GB pages"
    );
}
