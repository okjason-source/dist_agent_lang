...
fn test_runtime_transaction_lifecycle() {
    ...
    let tx_id = runtime.begin_transaction(IsolationLevel::Serializable, None)
        .expect("Failed to begin transaction");
    
    assert_eq!(runtime.current_transaction(), Some(tx_id.as_str()), "Transaction should be active");
    
    // Write some transactional state
    runtime.transaction_write("test_key".to_string(), Value::Int(42))
        .expect("Failed to write to transaction");
    
    // Read it back
    let value = runtime.transaction_read("test_key")
    ...
    assert_eq!(runtime.current_transaction(), None, "Transaction should be cleared after commit");
}
...
fn test_runtime_transaction_rollback() {
    ...
    runtime.begin_transaction(IsolationLevel::Serializable, None).unwrap();
    runtime.transaction_write("balance".to_string(), Value::Int(1000)).unwrap();
    runtime.commit_transaction().unwrap();
    
    // Start new transaction and modify state
    runtime.begin_transaction(IsolationLevel::Serializable, None).unwrap();
    runtime.transaction_write("balance".to_string(), Value::Int(500)).unwrap();
    ...
    assert_eq!(balance, Some(Value::Int(1000)), "Balance should be reverted to 1000 after rollback");
    runtime.commit_transaction().unwrap();
}
...
fn test_runtime_savepoint_and_partial_rollback() {
    ...
    // First write
    runtime.transaction_write("key1".to_string(), Value::String("value1".to_string())).unwrap();
    
    // Create savepoint
    runtime.create_savepoint("sp1".to_string())
        .expect("Failed to create savepoint");
    
    // Second write after savepoint
    runtime.transaction_write("key2".to_string(), Value::String("value2".to_string())).unwrap();
    
    // Rollback to savepoint (key2 should be removed, key1 kept)
    runtime.rollback_to_savepoint("sp1")
    ...
    assert_eq!(val1, Some(Value::String("value1".to_string())), "key1 should still exist");
...
fn test_transaction_isolation_levels() {
    ...
    for level in &[
        ...
        let _tx_id = runtime.begin_transaction(*level, None)
            .expect("Failed to begin transaction");
...
fn test_transaction_timeout_configuration() {
    ...
    let _tx_id = runtime.begin_transaction(IsolationLevel::Serializable, Some(5000))
        .expect("Failed to begin transaction with timeout");
    
    assert!(runtime.current_transaction().is_some());
    
    // Transaction should work normally within timeout
    runtime.transaction_write("key".to_string(), Value::Int(123)).unwrap();
    runtime.commit_transaction().unwrap();
}
...
fn test_multiple_transactions_sequential() {
    ...
    runtime.begin_transaction(IsolationLevel::Serializable, None).unwrap();
    runtime.transaction_write("counter".to_string(), Value::Int(0)).unwrap();
    runtime.commit_transaction().unwrap();
    ...
    if let Value::Int(n) = val {
        runtime.transaction_write("counter".to_string(), Value::Int(n + 1)).unwrap();
    }
    runtime.commit_transaction().unwrap();
    ...
    assert_eq!(final_val, Some(Value::Int(1)), "Counter should be incremented to 1");
    runtime.commit_transaction().unwrap();
}
