...
fn test_reentrancy_different_functions() {
    ...
    let result3 = guard.enter("function1", Some("contract1"));
    assert!(result3.is_err(), "Same function should be protected from reentrancy");
...
fn test_runtime_security_features() {
    ...
    assert!(result.is_ok(), "StateIsolationManager should be initialized and functional");
    assert!(runtime.state_manager.get_contract("0x1234").is_some(), "StateIsolationManager should create contracts");
