...
fn verify_hybrid_transaction(transaction: any) -> Result<VerificationResult, Error> {
    ...
    let crypto_check = crypto::verify_signatures(transaction);
    ...
    let final_decision = self.aggregate_verification_results([
        crypto_check, oracle_check, business_check, risk_check
    ]);
...
service HybridKeyManager {
    fn generate_hybrid_key_pair(user_id: String) -> Result<KeyPair, Error> {
        // Generate key pair
        let key_pair = crypto::generate_key_pair("secp256k1");

        // Store private key encrypted offchain
        let encrypted_private = crypto::encrypt_key(key_pair.private_key);

        // Store public key hash onchain
        let public_hash = crypto::hash(key_pair.public_key);
