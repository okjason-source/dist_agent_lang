...
fn test_lexer_large_file_performance() {
    ...
    assert!(duration.as_secs() < 5, "Lexer took too long: {:?}", duration);
}
...
fn test_parser_nested_structures_performance() {
    ...
    assert!(duration.as_secs() < 10, "Parser took too long: {:?}", duration);
}
...
fn test_runtime_many_variables() {
    ...
    assert!(duration.as_millis() < 1000, "Runtime creation took too long");
}
...
fn test_concurrent_reentrancy_guard() {
    ...
    println!("Reentrancy guard allowed {} out of 100 concurrent attempts", success_count);
...
fn test_lexer_malformed_inputs() {
    ...
    for (i, input) in malformed_inputs.iter().enumerate() {
        ...
        let result = lexer.tokenize();
        println!("Malformed input {}: {:?}", i, if result.is_ok() { "OK" } else { "Error" });
...
fn test_parser_error_recovery() {
    ...
    for (i, code) in invalid_codes.iter().enumerate() {
        ...
        if let Ok(tokens) = lexer.tokenize() {
            let mut parser = Parser::new(tokens);
            let result = parser.parse();
            println!("Invalid code {}: {:?}", i, if result.is_err() { "Error (expected)" } else { "OK" });
...
fn test_extreme_values() {
    ...
    assert!(max_i64.checked_mul(2).is_none(), "Should detect multiplication overflow");
