...
/// Solidity Contract Adapter
/// Provides utilities for integrating with Solidity contracts
...
pub fn parse_abi(abi_json: String) -> Result<Vec<ContractFunction>, String> {
    let abi: Vec<serde_json::Value> = serde_json::from_str(&abi_json)
    ...
    match solidity_type {
        ...
        _ => {
            ...
            if solidity_type.ends_with("[]") {
                "vector<".to_string() + &solidity_to_dal_type(&solidity_type[..solidity_type.len()-2]) + ">"
            } else {
                "any".to_string()
            }
        }
...
pub fn generate_wrapper_signature(function: &ContractFunction) -> String {
    ...
    for input in &function.inputs {
        signature.push_str(&format!(", {}: {}", input.name, solidity_to_dal_type(&input.param_type)));
    }
...
pub fn listen_to_event(
    ...
    crate::stdlib::log::info("event_listener", {
        let mut data = std::collections::HashMap::new();
        data.insert("contract".to_string(), Value::String(contract.address.clone()));
...
pub fn generate_wrapper_code(contract: &SolidityContract) -> Result<String, String> {
    let abi_json = contract.abi.as_ref()
    ...
    code.push_str(&format!("    contract_address: string = \"{}\";\n", contract.address));
    ...
    if !events.is_empty() {
        ...
        for event in &events {
            code.push_str(&format!("    fn on_{}(callback: function) {{\n", event.name));
            code.push_str(&format!("        solidity_adapter::listen_to_event(\n"));
            code.push_str(&format!("            self.contract_address,\n"));
            code.push_str(&format!("            \"{}\",\n", event.name));
